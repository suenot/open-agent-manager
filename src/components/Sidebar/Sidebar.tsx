...
export function Sidebar() {
  ...
  const handleDuplicateProject = async (projectId: string) => {
    try {
      const updated = await invoke<Project[]>("duplicate_project", { projectId });
      setProjects(updated);
    } catch (err) {
      addError("Projects", "Failed to duplicate project", String(err));
    }
  };
      ...
      >
        ...
        {contextMenu && (
          ...
          >
            ...
            <button
              onClick={() => handleDuplicateProject(contextMenu.project.id)}
              className="w-full px-3 py-2 text-left text-xs text-zinc-300 hover:bg-blue-500/20 hover:text-white flex items-center gap-2 transition-colors font-medium"
            >
